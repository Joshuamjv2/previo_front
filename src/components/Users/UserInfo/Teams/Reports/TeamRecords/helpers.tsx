...
export const computeRSI = (group) => {
    ...
    for (let i = 0; i < sliced.length - 1; i++) {
        ...
        if (
            normalize(a.measurement_id) === "d4ebb79e-a0a8-4550-8bc4-e4336b8490a3" &&
            normalize(b.measurement_id) === "f5daa493-5054-4ad2-97b0-d9db95e7cdd6"
        ) {
            b.rsi = a.results / b.results;
            b.ft = a.results; // Store the flight time value

            // Preserve exercise start flag
        ...
        if (
            normalize(a.measurement_id) === "f5daa493-5054-4ad2-97b0-d9db95e7cdd6" &&
            normalize(b.measurement_id) === "d4ebb79e-a0a8-4550-8bc4-e4336b8490a3"
        ) {
        ...
        if (
            normalize(a.measurement_id) === "f5daa493-5054-4ad2-97b0-d9db95e7cdd6" &&
            normalize(b.measurement_id) === "73e0ac8f-b5e8-44f3-9557-2db5bb98c8ce"
        ) {
            a.fd = b.results; // Store the flight distance value
            a.fd_units = b.units;
        ...
        if (
            normalize(b.measurement_id) === "f5daa493-5054-4ad2-97b0-d9db95e7cdd6" &&
            normalize(a.measurement_id) === "73e0ac8f-b5e8-44f3-9557-2db5bb98c8ce"
        ) {
            b.fd = a.results; // Store the flight distance value
            b.fd_units = b.units;
...
export const computePat = (group) => {
    ...
    for (let i = 0; i < sliced.length - 1; i++) {
        ...
        if (
            ...
            let contactTime = (measA === contact_id ? a.results : b.results) / 1000;
            let flightTime = (measA === flight_id ? a.results : b.results) / 1000;
            const pc = 1;
            const g = 9.806;
